...
class AuthTest:
    ...
    button_mainMENU = "//div[@class='header__menuIcon']"  # локатор переменной кнопки в главном меню для перехода по разделам
    mas_buttons_in_mainMENU = ["//div/a[@data-name='zastroyshchiki_menu']", "//div/a[@data-name='obekty_menu']",
                               ...
                               "//div/a/span[text()='О системе']"]  # массив(лист) кнопок перехода из главного меню в разделы АИС МЗ
...
def locate_tooltip():
    try:
        ...
        print('Ошибка элемента')  # Возвращает все знаки вопроса на виджетах основной страницы

class MainPageEnvironment:
    report_button = "//select[@id='VPtopTenSelect']"